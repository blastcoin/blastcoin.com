...
function Blastcoin_form_system_theme_settings_alter(&$form, &$form_state)
{
    ...
    $form['IMsettings']['color']['colors']                             = array(
        ...
        '#default_value' => theme_get_setting('colors', 'Blastcoin')
    );
  ...
  $form['IMsettings']['top_social_link'] = array(
    ...
    $form['IMsettings']['top_social_link']['twitter_username']    = array(
        ...
        '#default_value' => theme_get_setting('twitter_username', 'Blastcoin'),
    ...
    $form['IMsettings']['top_social_link']['facebook_username']   = array(
        ...
        '#default_value' => theme_get_setting('facebook_username', 'Blastcoin'),
    ...
    $form['IMsettings']['top_social_link']['googleplus_username'] = array(
        '#type' => 'textfield',
        '#title' => t('Google +'),
        '#default_value' => theme_get_setting('googleplus_username', 'Blastcoin'),
	...
	$form['IMsettings']['top_social_link']['youtube_username'] = array(
        '#type' => 'textfield',
        '#title' => t('YouTube'),
        '#default_value' => theme_get_setting('youtube_username', 'Blastcoin'),
	...
	$form['IMsettings']['top_social_link']['rss'] = array(
        '#type' => 'textfield',
        '#title' => t('RSS'),
        '#default_value' => theme_get_setting('rss', 'Blastcoin'),
    ...
    $form['IMsettings']['language_options']['language_links']     = array(
        ...
        '#default_value' => theme_get_setting('language_links', 'Blastcoin'),
    ...
    $form['banner']['slideshowSpeed']                             = array(
        ...
        '#default_value' => theme_get_setting('slideshowSpeed', 'Blastcoin')
    );
    ...
    foreach ($banners as $image_data) {
        $form['banner']['images'][$i] = array(
            ...
            'image' => _Blastcoin_banner_form($image_data)
        );
        $i++;
    }
    ...
    $form['#submit'][]              = 'Blastcoin_settings_submit';
    return $form;
}
/**
...
*/
function Blastcoin_settings_submit($form, &$form_state)
{
    $settings = array();
    // Update image field
    ...
    if ($file = file_save_upload('image_upload')) {
        ...
        if ($image = _Blastcoin_save_image($file)) {
            // Put new image into settings
            $settings[] = $image;
        }
    }
    // Save settings
    Blastcoin_set_banners($settings);
}
/**
...
*/
function _Blastcoin_install()
{
    // Deafault data
    $file            = new stdClass;
    $banners         = array();
    // Source base for images
    $src_base_path   = drupal_get_path('theme', 'Blastcoin');
    ...
    foreach ($default_banners as $i => $data) {
        ...
        $banner         = _Blastcoin_save_image($file);
        unset($data['image_path']);
        $banner      = array_merge($banner, $data);
        $banners[$i] = $banner;
    }
    // Save banner data
    Blastcoin_set_banners($banners);
    // Flag theme is installed
    variable_set('theme_Blastcoin_first_install', FALSE);
}
/**
...
*/
function _Blastcoin_save_image($file, $banner_folder = 'public://Blastcoin/slide/', $banner_thumb_folder = 'public://Blastcoin/slide/thumb/')
{
    // Check directory and create it (if not exist)
    _Blastcoin_check_dir($banner_folder);
    _Blastcoin_check_dir($banner_thumb_folder);
...
*/
function _Blastcoin_check_dir($dir)
{
...
*/
function _Blastcoin_banner_form($image_data)
{
    $img_form                      = array();
    // Image preview
